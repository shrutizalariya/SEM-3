...
--9. Give department wise maximum & minimum salary with department name. 
	SELECT D.DEPARTMENTNAME , MAX(P.SALARY) AS MAXIMUM_SALARY , MIN(P.SALARY) AS MINIMUM_SALARY
	FROM PERSON P
	INNER JOIN DEPT D
	ON P.DepartmentID = D.DepartmentID
	GROUP BY D.DepartmentName;
...
--10. Find city wise total, average, maximum and minimum salary. 
	SELECT CITY , MAX(SALARY) AS MAXIMUM_SALARY , MIN(SALARY) AS MINIMUM_SALARY , AVG(SALARY) AS AVERAGE_SALARY
	FROM PERSON
	GROUP BY CITY ;
...
--single column) 
	SELECT (P.PERSONNAME + ' LIVES IN ' + P.CITY + ' AND WORKS IN ' + D.DEPARTMENTNAME + ' DEPARTMENT.') AS SENTENCE
	FROM PERSON P
	INNER JOIN DEPT D
	ON P.DepartmentID = D.DepartmentID
...
--single column) 
	SELECT CONCAT(P.PERSONNAME , ' EARNS ' ,  P.SALARY , ' FROM ' , D.DEPARTMENTNAME , ' DEPARTMENT MONTHLY.') AS SENTENCE
	FROM PERSON P
	INNER JOIN DEPT D
	ON P.DepartmentID = D.DepartmentID
	
--2. Find city & department wise total, average & maximum salaries. 
	SELECT P.CITY , D.DPARTMENTNAME , SUM(P.SALARY) , AVG(P.SALARY) , MAX(P.SALARY)
